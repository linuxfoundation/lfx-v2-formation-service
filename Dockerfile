...
# Build the packages
RUN go build -o /go/bin/lfx-v2-formation-service -trimpath -ldflags="-w -s" github.com/linuxfoundation/lfx-v2-formation-service/cmd/formation-api
...
COPY --from=builder /go/bin/lfx-v2-formation-service /cmd/lfx-v2-formation-service

ENTRYPOINT ["/cmd/lfx-v2-formation-service"]
