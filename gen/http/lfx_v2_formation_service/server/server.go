// Code generated by goa v3.25.3, DO NOT EDIT.
//
// lfx_v2_formation_service HTTP server
//
// Command:
// $ goa gen
// github.com/linuxfoundation/lfx-v2-formation-service/cmd/formation-api/design

package server

import (
	"context"
	"net/http"

	lfxv2formationservice "github.com/linuxfoundation/lfx-v2-formation-service/gen/lfx_v2_formation_service"
	goahttp "goa.design/goa/v3/http"
	goa "goa.design/goa/v3/pkg"
)

// Server lists the lfx_v2_formation_service service endpoint HTTP handlers.
type Server struct {
	Mounts               []*MountPoint
	GetFormation         http.Handler
	GetFormationActivity http.Handler
	SetItemStatus        http.Handler
	AssignItem           http.Handler
	UpdateItem           http.Handler
	Livez                http.Handler
	Readyz               http.Handler
}

// MountPoint holds information about the mounted endpoints.
type MountPoint struct {
	// Method is the name of the service method served by the mounted HTTP handler.
	Method string
	// Verb is the HTTP method used to match requests to the mounted handler.
	Verb string
	// Pattern is the HTTP request path pattern used to match requests to the
	// mounted handler.
	Pattern string
}

// New instantiates HTTP handlers for all the lfx_v2_formation_service service
// endpoints using the provided encoder and decoder. The handlers are mounted
// on the given mux using the HTTP verb and path defined in the design.
// errhandler is called whenever a response fails to be encoded. formatter is
// used to format errors returned by the service methods prior to encoding.
// Both errhandler and formatter are optional and can be nil.
func New(
	e *lfxv2formationservice.Endpoints,
	mux goahttp.Muxer,
	decoder func(*http.Request) goahttp.Decoder,
	encoder func(context.Context, http.ResponseWriter) goahttp.Encoder,
	errhandler func(context.Context, http.ResponseWriter, error),
	formatter func(ctx context.Context, err error) goahttp.Statuser,
) *Server {
	return &Server{
		Mounts: []*MountPoint{
			{"GetFormation", "GET", "/formations/{project_uid}"},
			{"GetFormationActivity", "GET", "/formations/{project_uid}/activity"},
			{"SetItemStatus", "POST", "/formations/{project_uid}/items/{item_key}/status"},
			{"AssignItem", "POST", "/formations/{project_uid}/items/{item_key}/assignment"},
			{"UpdateItem", "PATCH", "/formations/{project_uid}/items/{item_key}"},
			{"Livez", "GET", "/livez"},
			{"Readyz", "GET", "/readyz"},
		},
		GetFormation:         NewGetFormationHandler(e.GetFormation, mux, decoder, encoder, errhandler, formatter),
		GetFormationActivity: NewGetFormationActivityHandler(e.GetFormationActivity, mux, decoder, encoder, errhandler, formatter),
		SetItemStatus:        NewSetItemStatusHandler(e.SetItemStatus, mux, decoder, encoder, errhandler, formatter),
		AssignItem:           NewAssignItemHandler(e.AssignItem, mux, decoder, encoder, errhandler, formatter),
		UpdateItem:           NewUpdateItemHandler(e.UpdateItem, mux, decoder, encoder, errhandler, formatter),
		Livez:                NewLivezHandler(e.Livez, mux, decoder, encoder, errhandler, formatter),
		Readyz:               NewReadyzHandler(e.Readyz, mux, decoder, encoder, errhandler, formatter),
	}
}

// Service returns the name of the service served.
func (s *Server) Service() string { return "lfx_v2_formation_service" }

// Use wraps the server handlers with the given middleware.
func (s *Server) Use(m func(http.Handler) http.Handler) {
	s.GetFormation = m(s.GetFormation)
	s.GetFormationActivity = m(s.GetFormationActivity)
	s.SetItemStatus = m(s.SetItemStatus)
	s.AssignItem = m(s.AssignItem)
	s.UpdateItem = m(s.UpdateItem)
	s.Livez = m(s.Livez)
	s.Readyz = m(s.Readyz)
}

// MethodNames returns the methods served.
func (s *Server) MethodNames() []string { return lfxv2formationservice.MethodNames[:] }

// Mount configures the mux to serve the lfx_v2_formation_service endpoints.
func Mount(mux goahttp.Muxer, h *Server) {
	MountGetFormationHandler(mux, h.GetFormation)
	MountGetFormationActivityHandler(mux, h.GetFormationActivity)
	MountSetItemStatusHandler(mux, h.SetItemStatus)
	MountAssignItemHandler(mux, h.AssignItem)
	MountUpdateItemHandler(mux, h.UpdateItem)
	MountLivezHandler(mux, h.Livez)
	MountReadyzHandler(mux, h.Readyz)
}

// Mount configures the mux to serve the lfx_v2_formation_service endpoints.
func (s *Server) Mount(mux goahttp.Muxer) {
	Mount(mux, s)
}

// MountGetFormationHandler configures the mux to serve the
// "lfx_v2_formation_service" service "get_formation" endpoint.
func MountGetFormationHandler(mux goahttp.Muxer, h http.Handler) {
	f, ok := h.(http.HandlerFunc)
	if !ok {
		f = func(w http.ResponseWriter, r *http.Request) {
			h.ServeHTTP(w, r)
		}
	}
	mux.Handle("GET", "/formations/{project_uid}", f)
}

// NewGetFormationHandler creates a HTTP handler which loads the HTTP request
// and calls the "lfx_v2_formation_service" service "get_formation" endpoint.
func NewGetFormationHandler(
	endpoint goa.Endpoint,
	mux goahttp.Muxer,
	decoder func(*http.Request) goahttp.Decoder,
	encoder func(context.Context, http.ResponseWriter) goahttp.Encoder,
	errhandler func(context.Context, http.ResponseWriter, error),
	formatter func(ctx context.Context, err error) goahttp.Statuser,
) http.Handler {
	var (
		decodeRequest  = DecodeGetFormationRequest(mux, decoder)
		encodeResponse = EncodeGetFormationResponse(encoder)
		encodeError    = EncodeGetFormationError(encoder, formatter)
	)
	return http.HandlerFunc(func(w http.ResponseWriter, r *http.Request) {
		ctx := context.WithValue(r.Context(), goahttp.AcceptTypeKey, r.Header.Get("Accept"))
		ctx = context.WithValue(ctx, goa.MethodKey, "get_formation")
		ctx = context.WithValue(ctx, goa.ServiceKey, "lfx_v2_formation_service")
		payload, err := decodeRequest(r)
		if err != nil {
			if err := encodeError(ctx, w, err); err != nil && errhandler != nil {
				errhandler(ctx, w, err)
			}
			return
		}
		res, err := endpoint(ctx, payload)
		if err != nil {
			if err := encodeError(ctx, w, err); err != nil && errhandler != nil {
				errhandler(ctx, w, err)
			}
			return
		}
		if err := encodeResponse(ctx, w, res); err != nil {
			if errhandler != nil {
				errhandler(ctx, w, err)
			}
		}
	})
}

// MountGetFormationActivityHandler configures the mux to serve the
// "lfx_v2_formation_service" service "get_formation_activity" endpoint.
func MountGetFormationActivityHandler(mux goahttp.Muxer, h http.Handler) {
	f, ok := h.(http.HandlerFunc)
	if !ok {
		f = func(w http.ResponseWriter, r *http.Request) {
			h.ServeHTTP(w, r)
		}
	}
	mux.Handle("GET", "/formations/{project_uid}/activity", f)
}

// NewGetFormationActivityHandler creates a HTTP handler which loads the HTTP
// request and calls the "lfx_v2_formation_service" service
// "get_formation_activity" endpoint.
func NewGetFormationActivityHandler(
	endpoint goa.Endpoint,
	mux goahttp.Muxer,
	decoder func(*http.Request) goahttp.Decoder,
	encoder func(context.Context, http.ResponseWriter) goahttp.Encoder,
	errhandler func(context.Context, http.ResponseWriter, error),
	formatter func(ctx context.Context, err error) goahttp.Statuser,
) http.Handler {
	var (
		decodeRequest  = DecodeGetFormationActivityRequest(mux, decoder)
		encodeResponse = EncodeGetFormationActivityResponse(encoder)
		encodeError    = EncodeGetFormationActivityError(encoder, formatter)
	)
	return http.HandlerFunc(func(w http.ResponseWriter, r *http.Request) {
		ctx := context.WithValue(r.Context(), goahttp.AcceptTypeKey, r.Header.Get("Accept"))
		ctx = context.WithValue(ctx, goa.MethodKey, "get_formation_activity")
		ctx = context.WithValue(ctx, goa.ServiceKey, "lfx_v2_formation_service")
		payload, err := decodeRequest(r)
		if err != nil {
			if err := encodeError(ctx, w, err); err != nil && errhandler != nil {
				errhandler(ctx, w, err)
			}
			return
		}
		res, err := endpoint(ctx, payload)
		if err != nil {
			if err := encodeError(ctx, w, err); err != nil && errhandler != nil {
				errhandler(ctx, w, err)
			}
			return
		}
		if err := encodeResponse(ctx, w, res); err != nil {
			if errhandler != nil {
				errhandler(ctx, w, err)
			}
		}
	})
}

// MountSetItemStatusHandler configures the mux to serve the
// "lfx_v2_formation_service" service "set_item_status" endpoint.
func MountSetItemStatusHandler(mux goahttp.Muxer, h http.Handler) {
	f, ok := h.(http.HandlerFunc)
	if !ok {
		f = func(w http.ResponseWriter, r *http.Request) {
			h.ServeHTTP(w, r)
		}
	}
	mux.Handle("POST", "/formations/{project_uid}/items/{item_key}/status", f)
}

// NewSetItemStatusHandler creates a HTTP handler which loads the HTTP request
// and calls the "lfx_v2_formation_service" service "set_item_status" endpoint.
func NewSetItemStatusHandler(
	endpoint goa.Endpoint,
	mux goahttp.Muxer,
	decoder func(*http.Request) goahttp.Decoder,
	encoder func(context.Context, http.ResponseWriter) goahttp.Encoder,
	errhandler func(context.Context, http.ResponseWriter, error),
	formatter func(ctx context.Context, err error) goahttp.Statuser,
) http.Handler {
	var (
		decodeRequest  = DecodeSetItemStatusRequest(mux, decoder)
		encodeResponse = EncodeSetItemStatusResponse(encoder)
		encodeError    = EncodeSetItemStatusError(encoder, formatter)
	)
	return http.HandlerFunc(func(w http.ResponseWriter, r *http.Request) {
		ctx := context.WithValue(r.Context(), goahttp.AcceptTypeKey, r.Header.Get("Accept"))
		ctx = context.WithValue(ctx, goa.MethodKey, "set_item_status")
		ctx = context.WithValue(ctx, goa.ServiceKey, "lfx_v2_formation_service")
		payload, err := decodeRequest(r)
		if err != nil {
			if err := encodeError(ctx, w, err); err != nil && errhandler != nil {
				errhandler(ctx, w, err)
			}
			return
		}
		res, err := endpoint(ctx, payload)
		if err != nil {
			if err := encodeError(ctx, w, err); err != nil && errhandler != nil {
				errhandler(ctx, w, err)
			}
			return
		}
		if err := encodeResponse(ctx, w, res); err != nil {
			if errhandler != nil {
				errhandler(ctx, w, err)
			}
		}
	})
}

// MountAssignItemHandler configures the mux to serve the
// "lfx_v2_formation_service" service "assign_item" endpoint.
func MountAssignItemHandler(mux goahttp.Muxer, h http.Handler) {
	f, ok := h.(http.HandlerFunc)
	if !ok {
		f = func(w http.ResponseWriter, r *http.Request) {
			h.ServeHTTP(w, r)
		}
	}
	mux.Handle("POST", "/formations/{project_uid}/items/{item_key}/assignment", f)
}

// NewAssignItemHandler creates a HTTP handler which loads the HTTP request and
// calls the "lfx_v2_formation_service" service "assign_item" endpoint.
func NewAssignItemHandler(
	endpoint goa.Endpoint,
	mux goahttp.Muxer,
	decoder func(*http.Request) goahttp.Decoder,
	encoder func(context.Context, http.ResponseWriter) goahttp.Encoder,
	errhandler func(context.Context, http.ResponseWriter, error),
	formatter func(ctx context.Context, err error) goahttp.Statuser,
) http.Handler {
	var (
		decodeRequest  = DecodeAssignItemRequest(mux, decoder)
		encodeResponse = EncodeAssignItemResponse(encoder)
		encodeError    = EncodeAssignItemError(encoder, formatter)
	)
	return http.HandlerFunc(func(w http.ResponseWriter, r *http.Request) {
		ctx := context.WithValue(r.Context(), goahttp.AcceptTypeKey, r.Header.Get("Accept"))
		ctx = context.WithValue(ctx, goa.MethodKey, "assign_item")
		ctx = context.WithValue(ctx, goa.ServiceKey, "lfx_v2_formation_service")
		payload, err := decodeRequest(r)
		if err != nil {
			if err := encodeError(ctx, w, err); err != nil && errhandler != nil {
				errhandler(ctx, w, err)
			}
			return
		}
		res, err := endpoint(ctx, payload)
		if err != nil {
			if err := encodeError(ctx, w, err); err != nil && errhandler != nil {
				errhandler(ctx, w, err)
			}
			return
		}
		if err := encodeResponse(ctx, w, res); err != nil {
			if errhandler != nil {
				errhandler(ctx, w, err)
			}
		}
	})
}

// MountUpdateItemHandler configures the mux to serve the
// "lfx_v2_formation_service" service "update_item" endpoint.
func MountUpdateItemHandler(mux goahttp.Muxer, h http.Handler) {
	f, ok := h.(http.HandlerFunc)
	if !ok {
		f = func(w http.ResponseWriter, r *http.Request) {
			h.ServeHTTP(w, r)
		}
	}
	mux.Handle("PATCH", "/formations/{project_uid}/items/{item_key}", f)
}

// NewUpdateItemHandler creates a HTTP handler which loads the HTTP request and
// calls the "lfx_v2_formation_service" service "update_item" endpoint.
func NewUpdateItemHandler(
	endpoint goa.Endpoint,
	mux goahttp.Muxer,
	decoder func(*http.Request) goahttp.Decoder,
	encoder func(context.Context, http.ResponseWriter) goahttp.Encoder,
	errhandler func(context.Context, http.ResponseWriter, error),
	formatter func(ctx context.Context, err error) goahttp.Statuser,
) http.Handler {
	var (
		decodeRequest  = DecodeUpdateItemRequest(mux, decoder)
		encodeResponse = EncodeUpdateItemResponse(encoder)
		encodeError    = EncodeUpdateItemError(encoder, formatter)
	)
	return http.HandlerFunc(func(w http.ResponseWriter, r *http.Request) {
		ctx := context.WithValue(r.Context(), goahttp.AcceptTypeKey, r.Header.Get("Accept"))
		ctx = context.WithValue(ctx, goa.MethodKey, "update_item")
		ctx = context.WithValue(ctx, goa.ServiceKey, "lfx_v2_formation_service")
		payload, err := decodeRequest(r)
		if err != nil {
			if err := encodeError(ctx, w, err); err != nil && errhandler != nil {
				errhandler(ctx, w, err)
			}
			return
		}
		res, err := endpoint(ctx, payload)
		if err != nil {
			if err := encodeError(ctx, w, err); err != nil && errhandler != nil {
				errhandler(ctx, w, err)
			}
			return
		}
		if err := encodeResponse(ctx, w, res); err != nil {
			if errhandler != nil {
				errhandler(ctx, w, err)
			}
		}
	})
}

// MountLivezHandler configures the mux to serve the "lfx_v2_formation_service"
// service "livez" endpoint.
func MountLivezHandler(mux goahttp.Muxer, h http.Handler) {
	f, ok := h.(http.HandlerFunc)
	if !ok {
		f = func(w http.ResponseWriter, r *http.Request) {
			h.ServeHTTP(w, r)
		}
	}
	mux.Handle("GET", "/livez", f)
}

// NewLivezHandler creates a HTTP handler which loads the HTTP request and
// calls the "lfx_v2_formation_service" service "livez" endpoint.
func NewLivezHandler(
	endpoint goa.Endpoint,
	mux goahttp.Muxer,
	decoder func(*http.Request) goahttp.Decoder,
	encoder func(context.Context, http.ResponseWriter) goahttp.Encoder,
	errhandler func(context.Context, http.ResponseWriter, error),
	formatter func(ctx context.Context, err error) goahttp.Statuser,
) http.Handler {
	var (
		encodeResponse = EncodeLivezResponse(encoder)
		encodeError    = goahttp.ErrorEncoder(encoder, formatter)
	)
	return http.HandlerFunc(func(w http.ResponseWriter, r *http.Request) {
		ctx := context.WithValue(r.Context(), goahttp.AcceptTypeKey, r.Header.Get("Accept"))
		ctx = context.WithValue(ctx, goa.MethodKey, "livez")
		ctx = context.WithValue(ctx, goa.ServiceKey, "lfx_v2_formation_service")
		var err error
		res, err := endpoint(ctx, nil)
		if err != nil {
			if err := encodeError(ctx, w, err); err != nil && errhandler != nil {
				errhandler(ctx, w, err)
			}
			return
		}
		if err := encodeResponse(ctx, w, res); err != nil {
			if errhandler != nil {
				errhandler(ctx, w, err)
			}
		}
	})
}

// MountReadyzHandler configures the mux to serve the
// "lfx_v2_formation_service" service "readyz" endpoint.
func MountReadyzHandler(mux goahttp.Muxer, h http.Handler) {
	f, ok := h.(http.HandlerFunc)
	if !ok {
		f = func(w http.ResponseWriter, r *http.Request) {
			h.ServeHTTP(w, r)
		}
	}
	mux.Handle("GET", "/readyz", f)
}

// NewReadyzHandler creates a HTTP handler which loads the HTTP request and
// calls the "lfx_v2_formation_service" service "readyz" endpoint.
func NewReadyzHandler(
	endpoint goa.Endpoint,
	mux goahttp.Muxer,
	decoder func(*http.Request) goahttp.Decoder,
	encoder func(context.Context, http.ResponseWriter) goahttp.Encoder,
	errhandler func(context.Context, http.ResponseWriter, error),
	formatter func(ctx context.Context, err error) goahttp.Statuser,
) http.Handler {
	var (
		encodeResponse = EncodeReadyzResponse(encoder)
		encodeError    = EncodeReadyzError(encoder, formatter)
	)
	return http.HandlerFunc(func(w http.ResponseWriter, r *http.Request) {
		ctx := context.WithValue(r.Context(), goahttp.AcceptTypeKey, r.Header.Get("Accept"))
		ctx = context.WithValue(ctx, goa.MethodKey, "readyz")
		ctx = context.WithValue(ctx, goa.ServiceKey, "lfx_v2_formation_service")
		var err error
		res, err := endpoint(ctx, nil)
		if err != nil {
			if err := encodeError(ctx, w, err); err != nil && errhandler != nil {
				errhandler(ctx, w, err)
			}
			return
		}
		if err := encodeResponse(ctx, w, res); err != nil {
			if errhandler != nil {
				errhandler(ctx, w, err)
			}
		}
	})
}
